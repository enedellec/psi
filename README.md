...
- Data for the tests are available in the `data` folder and generated by the GO program located in the `data-generation` folder.
- Each file contains a list of sorted SHA256 hashes.
- For testing, the goal is to use two files with the same number of items, with the half of items in common.
...
- For more information on the generation of data, you just need to have a look at the `main.go` file in the `data-generation` folder.

# PSI on a local computer without any TEE support
## PSI with a third-party, but without any TEE support
- **Prerequisite:** you must [install GO](https://go.dev/doc/install), if it is not already installed on your local machine.
...
- The content of the `result.txt` file should be equal to the intersection between `data-100-all.csv` and `data-100-even-only.csv` files. To validate the result provided by the PSI server, you can try that command:
```
...
## PSI with SMPC
- **Prerequisites:**
    - you must [install Python](https://docs.python.org/fr/3/using/unix.html), if it is not already installed on your local machine.
    - you are located in the folder where the PSI git project have been cloned (cf. previous section).
- Start by cloning the libPSI project: 
```
git clone https://github.com/osu-crypto/libPSI.git
```
- At this stage, both PSI and libPSI projects are located in the same folder.
- Build the libPSI project:
```
cd libPSI
python build.py
```
...
- Launch the `deploy.sh` script in order to create two differents SGX enabled VM.
...
- The steps to follow are very similar to the ones for testing on your local machine, except that some steps are done within the `install.sh` script file.
...
ego run server
```
- Then, open another terminal on the same VM, and build the client with a recent GO compiler:
```
cd ~/psi/with-enclaves
